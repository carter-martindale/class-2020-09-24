...
```{r scene_1}
sep_both_inc <- sep_new %>% 
  inner_join(sep_old, by = c("id", "title", "name",
                             "department"), 
             suffix = c("_new", "_old")) %>% 
...
```{r c}
sep_total <- rbind(sep_both_inc, sep_both_dec)
view(sep_total)

sep_total %>% 
  ggplot(aes(x = title, y = enrollment_change, fill = name)) +
  geom_col() +
...
# units could be classes
# survey the kids, ask how much coursework they have
# treated could be absurd amount of work, so courses are either yt (tons of work) or yc (normal amount)
# you can then fill in enrollment based on those classes, both initially and then on september 1st
